// One-off regen for the $TOSHI tweet image, using the proper toshi.png reference
// (chibi blue fox-like Base mascot in orange hoodie). Uses persistent X Tweets chat
// per image-gen-chat-reuse rule. Safe in parallel with non-chatgpt-profile scripts.
...
(async () => {
  console.log('Toshi regen — using toshi.png reference, fresh image_id ' + NEW_UUID);
  ...
  try {
    await generateOne(page, { targetPath: newPath, prompt: PROMPT, refImage: TOSHI_REF });
  } catch (err) {
  ...
  const tweet = data.tweets.find(t => /^\$TOSHI is named after the Coinbase CEO/.test(t.tweet || ''));
  if (!tweet) {
    console.error('Could not find Toshi tweet in x-tweets.json');
    await browser.close();
    process.exit(1);
  }
