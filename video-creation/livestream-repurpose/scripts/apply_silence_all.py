...
SLUGS = ["xrp-vs-kaspa","kaspa-iso20022-swift","kaspa-entry-doesnt-matter","ideas-in-a-cemetery",
 "ai-white-collar-first","webcam-girl-rug-cycle","why-200-keeps-rugging","no-new-memes-bear-market",
 "microstrategy-for-ton","stablecoin-yield-fight","sui-favorites-etfs","pengu-flips-pepe",
 "hunt-a-virus-rug","coinmarketcap-test","human-driving-illegal"]
...
print("Applying silence removal (-57/-52, >250ms gaps) to all 15 clips:\n")
for slug in SLUGS:
    f=os.path.join(SHORTS,slug,"preview.mp4")
    if not os.path.exists(f): print(f"  {slug}: MISSING"); continue
    b=dur(f); n=remove_silence(f); a=dur(f)
    print(f"  {slug:30s} {b:6.1f}s -> {a:6.1f}s  (-{b-a:4.1f}s, {n} regions)")
print("\nDone. Rebuild/refresh the dashboard to review.")
