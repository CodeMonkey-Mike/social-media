"""Fast single-pass silence removal for the 'whatif' longform (built for 60+ min; delete_silences.py
is per-region and impractical at that scale). silencedetect -> keep intervals -> ONE ffmpeg NVENC
pass via the concat demuxer, dropping the gaps. Outputs ~0.7 Mbps directly to the staged location.
Modeled verbatim on longform_desilence_fast.py.
"""
import subprocess, re, os, sys

SRC = r"C:\Users\mnede\Documents\Claude\social-media\video-creation\livestream-repurpose\media\whatif\whatif LOW BPS.mp4"
OUT = r"C:\Users\mnede\Documents\Claude\social-media\schedule-tweets\longform\whatif\whatif.mp4"
NOISE = "-50dB"   # below this = silence
MIN_SIL = 0.5     # only cut gaps longer than this
...
print(f"{len(keeps)} keep spans; building single-pass NVENC concat")
