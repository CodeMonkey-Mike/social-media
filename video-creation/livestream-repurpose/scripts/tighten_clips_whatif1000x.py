"""Phase 5 tighten + Phase 5B desilence for the 'what-if-1000x' batch.

Reads the tighten-strategist's plan at shorts/what-if-1000x/tighten-plan.json, executes it against
the VERTICAL MASTER at absolute timestamps (never against the preview), then runs the ONE canonical
desilencer on the result. Modeled on tighten_clips_october_pumps.py (the reference).
...
Batch specifics:
  * clip-plan.json keys clips by `slug` (+ integer `clip_id`); the dashboard number `n` is STABLE
    (Mike deleted clip 8 on 2026-08-03; survivors 1-7 do NOT renumber).
  * per-clip hook context lives on the TOPIC (topic_id -> hook_summary), not on the clip.
  * clips 2, 3, 4 are deliberately NON-CHRONOLOGICAL (assembly_order [3,1,2,0], [1,0], [2,3,1,4,0]).
"""
import json, os, subprocess, sys, tempfile, shutil

REPO = r"C:\Users\mnede\Documents\Claude\social-media"
BATCH = "what-if-1000x"
MASTER = os.path.join(REPO, r"video-creation\livestream-repurpose\media\what-if-1000x\what-if-1000x LOW BPS VERTICAL.mp4")
...
TRANSCRIPT = os.path.join(REPO, r"video-creation\livestream-repurpose\transcripts"
                                r"\what-if-1000x LOW BPS VERTICAL\what-if-1000x LOW BPS VERTICAL.json")
...
print("== Phase 5 tighten + 5B desilence (what-if-1000x) ==", flush=True)

# Optional clip filter: `python tighten_clips_whatif1000x.py <slug> [<slug> ...]` re-renders only
# those clips and reuses the previous tighten_log entries for the rest (single-clip corrections
# never force a full 7-clip re-render). The dashboard is still rebuilt from the COMPLETE set.
only = set(sys.argv[1:])
prev_log = {}
...
for t in tighten["clips"]:
    ...
    # segments IN ASSEMBLY ORDER (clips 2/3/4 are deliberately non-chronological)
...
path = build_dashboard(
    BATCH, OUTDIR, cards,
    title="what-if-1000x (What If 1000x / October bottom / LAB 353x, 2026-08-02)",
    subtitle_extra="Phase 5 tightened + 5B desilenced; clip 8 deleted 2026-08-03 (numbers do NOT "
                   "renumber); awaiting Mike's 2nd review before captions/Remotion")
