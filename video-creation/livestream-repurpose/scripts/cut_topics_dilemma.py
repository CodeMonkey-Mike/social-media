"""
Cut preview clips for the "4-year cycle dilemma LOW BPS VERTICAL" livestream (Phase 4b).
Source is the Phase-1 vertical (content top + face bottom) - no crop, just trim.
This batch uses EXPLICIT second-based boundaries (the raw Whisper JSON has mishearings
- 'Casper'=Kaspa, 'four-year'->'for your' - so phrase anchoring is unreliable; times were
pinned from the word-level file). RE-ENCODE every segment (A/V sync: never -c copy across a
trim). Emits dashboard + progress.json and registers the batch.

3 topics, 4 clips (Topic 1 ships a long cut + a punch sub-clip within it).
"""
...
TOPICS = [
    {"slug": "jobs-so-good-it-was-bad", "title": "The Jobs Report That Broke Bitcoin",
     "hook": "Every jobs report this week was phenomenal. It was so good the market read it as a rate-hike signal, sold off, and Bitcoin finally fell below the 200-week SMA.",
     "start": 541, "end": 642},
    {"slug": "so-good-it-was-bad-punch", "title": "So Good It Was Bad (punch)",
     "hook": "172k jobs when they expected 85k. It was so goddamn good the market interpreted it as BAD, and people started selling off.",
     "start": 569, "end": 602},
    {"slug": "four-year-cycle-not-magic", "title": "The Four-Year Cycle Isn't Magic",
     "hook": "People keep telling me the four-year cycle zombies were right. I called a crypto winter back in August; I just never called a 2025 cycle top. It's all explainable.",
     "start": 946, "end": 996},
    {"slug": "kaspa-to-3-boy-was-i-wrong", "title": "I Thought Kaspa Would Hit $3. Boy Was I Wrong",
     "hook": "I bought Kaspa at 10-11 cents thinking it would hit $3 by the end of 2025. Boy, was I wrong. At least I realized early; some people got blindsided.",
     "start": 1168, "end": 1248},
]
...
<title>4-year cycle dilemma - Draft Clips</title>
<style>
...
<header>
  <h1>4-year cycle dilemma - Draft Clips</h1>
  <p>4 candidate clips (3 topics; Topic 1 has a long cut + a punch) &nbsp;&middot;&nbsp; Raw cuts (no captions/b-roll yet) &nbsp;&middot;&nbsp; Watch each, then Approve or Skip</p>
</header>
...
for i, t in enumerate(TOPICS, 1):
    clips.append({
        ...
        "source_times": [f"{fmt(t['start'])}-{fmt(t['end'])}"],
        "multi": False,
...
progress = {
    ...
    "dashboard": "shorts/dilemma/dashboard.html",
    "dashboard_status": "Built 4 raw preview clips (3 topics; Topic 1 = long + punch). AWAITING Mike's review/approval - do NOT start production until clips are approved.",
...
register_batch(
    batch="dilemma", date="2026-06-07", livestream_title=NAME,
    source_media="video-creation/livestream-repurpose/media/4-year cycle dilemma/" + NAME + ".mp4",
    transcripts_dir="video-creation/livestream-repurpose/transcripts/" + NAME,
    dashboard="video-creation/shorts/dilemma/dashboard.html",
