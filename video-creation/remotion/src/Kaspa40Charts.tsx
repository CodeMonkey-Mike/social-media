...
/**
 * kaspa 30bps — CHARTS H1 / C2 / C3 / FIN (Type 1 ANIMATED).
 *
 * Ported 1:1 from the LOCKED design sources in
 *   media/kaspa 30bps/assets/charts/{h1,c2,c3,fin}.html  (+ the *-state PNG stills)
 * The HTML/CSS is the layout+type spec; the motion below is the real animation.
 *
 * Contract for each:
 *   <H1 ts={sourceSeconds} />  — ts is ABSOLUTE e-spine SOURCE seconds (pre-card-pause).
 *   Full-frame 1920x1080 AbsoluteFill, no <Sequence> inside, all motion from `ts` alone.
 */
...
// ─── palette (:root of every locked chart HTML) ───────────────────────────────
const BG = '#0a0c10';
const CARD = '#12151c';
const GREEN = '#00e68a';
const CYAN = '#00c2ff';
...
// ─── shared frame chrome (.frame + .frame::after grain) ───────────────────────
...
const Orb: React.FC<OrbProps> = ({ size, color, opacity, blur = 120, left, right, top, bottom, center }) => (
  <div
    style={{
      position: 'absolute',
...
// Header block shared by C2 / C3 / FIN (header{left:90;top:56}).
const ChartHeader: React.FC<{
  eyebrow: string; children: React.ReactNode; op: number; dy: number;
}> = ({ eyebrow, children, op, dy }) => (
  <div
    style={{
  ...
  >
    <div
      style={{
        fontFamily: DM, fontSize: 26, fontWeight: 600, textTransform: 'uppercase',
    ...
    <h1
      style={{
        margin: 0, fontFamily: PF, fontWeight: 900, fontSize: 66,
...
const Foot: React.FC<{ children: React.ReactNode; op: number }> = ({ children, op }) => (
  <div
    style={{
      position: 'absolute', left: 90, bottom: 44, fontFamily: DM, fontSize: 20,
      color: MUTED, zIndex: 1, opacity: op,
    }}
  >
    {children}
  </div>
...
export const H1: React.FC<{ ts: number }> = ({ ts }) => {
  const u = ts - H1_IN;
  const s = ts - H1_SLAM;
  const slam = s >= 0;
  ...
  return (
    <Frame>
      <Orb
        size={640} color={CYAN} blur={130} left="50%" top="40%" center
        opacity={(slam ? ip(s, [0, 0.12], [0.17, 0]) : 0.17) * inOp}
      />
      <Orb
        size={700} color={GREEN} blur={130} left="50%" top="40%" center
      ...
      >
        <div
          style={{
            fontFamily: DM, fontSize: 27, fontWeight: 600, textTransform: 'uppercase',
        ...
        <div
          style={{
            height: 430, display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        >
          {slam ? (
            ...
            >
              <span
                style={{
                  fontFamily: DM, fontWeight: 700, fontSize: 64, letterSpacing: '.18em',
                  color: TEXT2, textTransform: 'uppercase', alignSelf: 'center', marginRight: 44,
                  opacity: ip(s, [0.02, 0.26], [0, 1], OUT),
                  transform: `translateX(${ip(s, [0.02, 0.32], [-70, 0], OUT)}px)`,
                }}
              >
                Up to
              </span>
              <span
                style={{
                  fontFamily: JB, fontWeight: 700, fontSize: 430, lineHeight: 0.95,
                  letterSpacing: '-.02em', color: GREEN,
                  textShadow: `0 0 ${glowBlur}px rgba(0,230,138,.4)`,
                }}
              >
                40
              </span>
            ...
            <span
              style={{
                fontFamily: JB, fontWeight: 700, fontSize: 430, lineHeight: 0.95,
                letterSpacing: '-.02em', color: TEXT,
                textShadow: `0 0 ${glowBlur}px rgba(0,194,255,.35)`,
        ...
        <div
          style={{
            fontFamily: DM, fontWeight: 700, fontSize: 66, letterSpacing: '.32em',
            textTransform: 'uppercase', color: TEXT, marginTop: 34,
          }}
        >
          Blocks / Sec
        </div>
        ...
        {!slam && (
          <div style={{ display: 'flex', gap: 20, marginTop: 44, height: 22, alignItems: 'center' }}>
              ...
              return (
                <span
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${on ? CYAN : 'rgba(0,194,255,.35)'}`,
                    boxShadow: on ? `0 0 ${22 + 16 * fresh}px rgba(0,194,255,.8)` : 'none',
        ...
        <div
          style={{
            fontFamily: JB, fontWeight: 600, fontSize: 30, color: TEXT2, marginTop: 34,
      ...
      {st >= 0 && (
        <div
          style={{
            position: 'absolute', right: 150, top: 150, zIndex: 2,
            transform: `rotate(${ip(st, [0, 0.28], [-21, -7], OUT)}deg) scale(${ip(
...
export const FIN: React.FC<{ ts: number }> = ({ ts }) => {
  const u = ts - FIN_IN;
  const c = ts - FIN_COL;
  const collapse = c >= 0;
  ...
  // the label rides the collapsing bar's right edge, clamped so it never runs off frame
  const val2X = Math.min(w2, 60);
  ...
  return (
    <Frame>
      <Orb size={520} color={CYAN} opacity={0.14} right={-160} top={-170} />
      <Orb size={520} color={GREEN} opacity={0.13} left={-180} bottom={-190} />

      <ChartHeader eyebrow="Finality" op={headOp} dy={headDy}>
        How long until a transaction is <span style={{ color: GREEN }}>final</span>
      </ChartHeader>

      <div style={{ position: 'absolute', left: 90, right: 90, top: 330, zIndex: 1, opacity: rowsOp }}>
        {/* row1 — Today */}
        <div style={{ marginBottom: 70, opacity: row1Dim }}>
          <div
            style={{
              fontFamily: DM, fontWeight: 700, fontSize: 32, marginBottom: 18,
              display: 'flex', alignItems: 'center', gap: 26, height: 56,
            }}
          >
            Today
          </div>
          <div
            style={{
              height: 96, borderRadius: 16, background: CARD, border: `1px solid ${BORDER}`,
          ...
          >
            ...
            >
              <span
                style={{
                  fontFamily: JB, fontWeight: 700, fontSize: 42, whiteSpace: 'nowrap',
                  color: '#04060d', marginLeft: 'auto', marginRight: 30,
        ...
        <div>
          ...
          >
            ...
            {chip >= 0 && (
              <span
                style={{
                  fontFamily: JB, fontWeight: 700, fontSize: 22, letterSpacing: '.08em',
                  padding: '10px 22px', borderRadius: 100, textTransform: 'uppercase',
          ...
          >
            ...
            <span
              style={{
                ...
                transform: 'translateY(-50%)', fontFamily: JB, fontWeight: 700, fontSize: 42,
        ...
        <div
          style={{
            ...
            fontFamily: JB, fontSize: 20, color: MUTED, opacity: axisOp,
          }}
        >
...
export const C2: React.FC<{ ts: number }> = ({ ts }) => {
  const u = ts - C2_IN;
  const b = ts - C2_BLINK;
  const headOp = ip(u, [0, 0.35], [0, 1], OUT);
  const headDy = ip(u, [0, 0.45], [-26, 0], OUT);
  ...
  return (
    <Frame>
      <Orb size={520} color={GREEN} opacity={0.13} right={-160} top={-170} />
      <Orb size={500} color={CYAN} opacity={0.13} left={-170} bottom={-190} />

      <ChartHeader eyebrow="Block cadence" op={headOp} dy={headDy}>
        <span style={{ color: GREEN }}>One second</span> of block production
      </ChartHeader>

      <div style={{ position: 'absolute', left: 90, right: 90, top: 300, zIndex: 1 }}>
        {C2_LANES.map((ln, i) => {
          ...
          return (
            <div
              key={ln.name}
              style={{
                display: 'flex', alignItems: 'center', gap: 40, marginBottom: 44,
                opacity: rowOp, transform: `translateX(${rowDx}px)`,
              }}
            >
              <div style={{ width: 430, flex: 'none' }}>
                <div style={{ fontFamily: DM, fontWeight: 700, fontSize: 32, letterSpacing: '.02em' }}>
                  {ln.name}
                </div>
                <div style={{ fontFamily: JB, fontWeight: 600, fontSize: 22, color: MUTED, marginTop: 6 }}>
                  {ln.int}
                </div>
              </div>
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                <div
                  style={{
                    height: 74, borderRadius: 14, background: CARD,
                    ...
                    <>
                      <div
                        style={{
                          ...
                          paddingLeft: 26, fontFamily: JB, fontWeight: 600, fontSize: 22,
                    ...
                    new Array(nTicks).fill(0).map((_, k) => {
                      ...
                      return (
                        <div
                          key={k}
                          style={{
                            position: 'absolute', top: 14, bottom: 14, width: 7, borderRadius: 4,
                            left: `calc(${k * (ln.step ?? 10)}% + ${nTicks === 10 ? 12 : 6}px)`,
                ...
                {ln.ticks === 40 && b >= 0 && (
                  <div
                    style={{
                      ...
                      border: `3px solid ${GOLD}`, borderRadius: 12, background: 'rgba(255,215,0,.06)',
              ...
              <div
                style={{
                  width: 210, flex: 'none', textAlign: 'right', fontFamily: JB, fontWeight: 700,
                  fontSize: 34, color: ln.countColor, opacity: countOp,
                  transform: `scale(${countScale})`, transformOrigin: 'right center',
                }}
              >
                {ln.count}
                <small
                  style={{
                    display: 'block', fontSize: 18, fontWeight: 600, color: MUTED,
                    marginTop: 4, letterSpacing: '.1em',
                  }}
                >
                  THIS SECOND
                </small>
        ...
        <div
          style={{
            display: 'flex', justifyContent: 'space-between', margin: '2px 0 26px 470px',
            paddingRight: 250, fontFamily: JB, fontSize: 19, color: MUTED,
      ...
      {b >= 0 && (
        <>
          <div style={{ position: 'absolute', right: 90, top: 180, zIndex: 3, textAlign: 'right' }}>
            <div
              style={{
                fontFamily: JB, fontWeight: 700, fontSize: 54, color: GOLD,
                ...
                transform: `rotate(${ip(b, [0.04, 0.34], [-15, -3], OUT)}deg) scale(${ip(
          ...
          <div
            style={{
              position: 'absolute', left: 560, top: 882, zIndex: 3, fontFamily: DM,
              fontWeight: 600, fontSize: 24, color: GOLD,
...
export const C3: React.FC<{ ts: number }> = ({ ts }) => {
  const u = ts - C3_IN;
  const d = ts - C3_DEMO;
  const cap = ts - C3_CAP;
  ...
  return (
    <Frame>
      <Orb size={520} color={GREEN} opacity={0.13} right={-160} top={-170} />
      <Orb size={500} color={GOLD} opacity={0.1} left={-170} bottom={-190} />

      <ChartHeader eyebrow="Transactions per second · capacity" op={inOp} dy={headDy}>
        What each block rate <span style={{ color: GREEN }}>can carry</span>
      </ChartHeader>

      <div
        style={{
          position: 'absolute', left: 90, right: 90, top: 300, zIndex: 1,
        ...
        {C3_ROWS.map((r, i) => {
          ...
          return (
            <div
              key={r.bps}
              style={{
                display: 'flex', alignItems: 'center', gap: 40, marginBottom: 52,
                position: 'relative', opacity: rowOp,
            ...
            >
              <div style={{ width: 330, flex: 'none' }}>
                <div
                  style={{
                    fontFamily: JB, fontWeight: 700, fontSize: 33,
                ...
                <div
                  style={{
                    fontFamily: DM, fontWeight: 600, fontSize: 20, color: MUTED, marginTop: 6,
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                <div
                  style={{
                    height: 58, borderRadius: 12, background: CARD,
                    ...
                    {r.inside && (
                      <span
                        style={{
                          ...
                          fontFamily: JB, fontWeight: 700, fontSize: 32, color: '#04060d',
                          whiteSpace: 'nowrap', opacity: ip(p, [0.3, 0.55], [0, 1]),
                        }}
                      >
                        {fmt(r.val * p)}+
                        <small style={{ fontSize: 20, color: 'rgba(4,6,13,.7)', fontWeight: 600, marginLeft: 10 }}>
                          TPS
                        </small>
                      </span>
                    )}
                  ...
                  {!r.inside && (
                    <div
                      style={{
                        ...
                        fontSize: 32, color: TEXT, whiteSpace: 'nowrap',
                        opacity: ip(p, [0.06, 0.28], [0, 1]),
                      }}
                    >
                      {fmt(r.val * p)}+
                      <small style={{ fontSize: 20, color: TEXT2, fontWeight: 600, marginLeft: 10 }}>TPS</small>
                    </div>
                  )}
                </div>

                {/* DEMONSTRATED marker — stamps onto the 10 BPS row */}
                {r.demo && d >= 0 && (
                  <>
                    <div
                      style={{
                        position: 'absolute', left: `calc(${DEMO_PCT}% - 2px)`, top: -56, bottom: -12,
                        width: 4, borderRadius: 2, background: GREEN,
                        boxShadow: '0 0 18px rgba(0,230,138,.7)', zIndex: 3,
                        transform: `scaleY(${ip(d, [0, 0.22], [0, 1], OUT)})`, transformOrigin: 'bottom center',
                      }}
                    />
                    <div
                      style={{
                        position: 'absolute', left: `calc(${DEMO_PCT}% + 16px)`, top: -60, zIndex: 3,
                        whiteSpace: 'nowrap', fontFamily: JB, fontWeight: 700, fontSize: 23, color: GREEN,
      ...
      {cap >= 0 && (
        <div
          style={{
            position: 'absolute', left: '50%', top: '52%', width: 960, zIndex: 5, textAlign: 'center',
        ...
        >
          <div
            style={{
              fontFamily: JB, fontWeight: 700, fontSize: 96, letterSpacing: '.04em', color: GOLD,
              border: `8px solid ${GOLD}`, borderRadius: 20, padding: '26px 54px',
          ...
          <div
            style={{
              fontFamily: DM, fontWeight: 600, fontSize: 30, color: TEXT2, marginTop: 26,
              display: 'inline-block', background: 'rgba(10,12,16,.88)',
              border: `1px solid ${BORDER}`, borderRadius: 12, padding: '14px 28px',
...
export const ChartsPreview: React.FC = () => {
  const frame = useCurrentFrame();
  const ts = frame / 30;
  if (ts < 40) return <H1 ts={ts} />;
  if (ts < 240) return <FIN ts={ts} />;
  if (ts < 347.32) return <C2 ts={ts} />;
  return <C3 ts={ts} />;
};
