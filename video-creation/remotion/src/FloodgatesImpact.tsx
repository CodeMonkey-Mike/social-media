...
import {
  FGI_FPS, FGI_SEAM, FGI_CAP_Y, CLIP, THUMB, THUMB_TITLE, THUMB_CHIP,
  CAPTIONS_FGI, BROLL_FGI, SFX_FGI,
} from './constants-fgi';
...
// visible below. This clip is 3 isolated full-screens (hook / floodgates / six-billion): none are
// adjacent (base gaps >> 1.5s), so each fades to/from the base — no full-to-full base flash.
const BrollLayer: React.FC<{ t: number }> = ({ t }) => {
  const idx = BROLL_FGI.findIndex(e => t >= e.tIn && t < e.tOut);
  if (idx < 0) return null;
  const ev = BROLL_FGI[idx];
  const F = 0.12, EPS = 0.18;
  const prevAdj = BROLL_FGI.some((o, i) => i !== idx && Math.abs(o.tOut - ev.tIn) <= EPS);
  const nextAdj = BROLL_FGI.some((o, i) => i !== idx && Math.abs(o.tIn - ev.tOut) <= EPS);
  ...
  return (
    <AbsoluteFill style={{ opacity: op, zIndex: 60 }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 1080, height: FGI_SEAM, overflow: 'hidden' }}>
        <Img src={ev.src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${kb})` }} />
...
const Caption: React.FC = () => {
  ...
  const t = frame / FGI_FPS;
  let idx = 0, html = '';
  for (let i = CAPTIONS_FGI.length - 1; i >= 0; i--) {
    if (t >= CAPTIONS_FGI[i].t) { idx = i; html = CAPTIONS_FGI[i].h; break; }
  }
  if (!html) return null;
  const startFrame = Math.round((CAPTIONS_FGI[idx]?.t ?? 0) * FGI_FPS);
  const scale = spring({ frame: frame - startFrame, fps: FGI_FPS, config: { damping: 11, stiffness: 360 }, from: 0.72, to: 1.0 });
  return (
    <div style={{
      position: 'absolute', top: FGI_CAP_Y, left: 46, right: 46,
...
const ThumbCover: React.FC<{ op: number }> = ({ op }) => (
  <AbsoluteFill style={{ zIndex: 400, opacity: op }}>
    ...
    <AbsoluteFill style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.82) 0%, rgba(0,0,0,0.10) 34%, rgba(0,0,0,0.20) 60%, rgba(0,0,0,0.88) 100%)' }} />
    {/* Headline at top, chip near the bottom — so the floodgates hero stays visible between them. */}
    <div style={{ position: 'absolute', top: 92, left: 50, right: 50, textAlign: 'center' }}>
      <div style={{
        fontFamily: FONT, fontWeight: 900, fontSize: 116, lineHeight: 0.98, color: '#fff',
        textTransform: 'uppercase', WebkitTextStroke: '5px #000', paintOrder: 'stroke fill' as any,
        textShadow: `0 0 44px ${RH_NEON}66`,
      }}>{THUMB_TITLE}</div>
    </div>
    <div style={{ position: 'absolute', bottom: 430, left: 0, right: 0, textAlign: 'center' }}>
      <div style={{
        display: 'inline-block', background: RH_NEON, color: '#0a1000',
        fontFamily: FONT, fontWeight: 900, fontSize: 52, letterSpacing: '0.02em',
        padding: '20px 44px', borderRadius: 18, boxShadow: `0 0 48px ${RH_NEON}`,
      }}>{THUMB_CHIP}</div>
    </div>
  </AbsoluteFill>
);
...
// ─── Main composition ─────────────────────────────────────────────────────────────
export const FloodgatesImpact: React.FC = () => {
  const frame = useCurrentFrame();
  const t = frame / FGI_FPS;
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT, overflow: 'hidden' }}>
      ...
      {/* Layer 1 — b-roll (full-screen at hook / floodgates climax / six-billion button) */}
      <BrollLayer t={t} />

      {/* Layer 2 — caption legibility band straddling the seam */}
      <div style={{
        position: 'absolute', top: FGI_SEAM - 96, left: 0, right: 0, height: 216, zIndex: 120,
      ...
      {/* Layer 5 — SFX (whoosh / riser / boom / impacts / kick / cash register), each under the VO */}
      {SFX_FGI.map((s, i) => (
        <Sequence key={i} from={Math.round(s.t * FGI_FPS)} durationInFrames={Math.max(1, Math.round(s.dur * FGI_FPS))}>
