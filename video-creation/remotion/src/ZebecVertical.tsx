...
import { DemandVsFloatV, TractionScoreboardV, BuybackFlywheelV } from './zebecChartsVertical';
...
// ZebecVertical — 1080x1920 reframe of the approved 16:9 (vertical-repurpose.md). Same spine/sh/duration.
export const ZV_FPS = 30;
const SPINE_SECS = 458.54;
export const ZV_DURATION = Math.round(SPINE_SECS * ZV_FPS);
const ease = Easing.out(Easing.cubic);
const F = (t: number) => Math.round(t * ZV_FPS);
const GREEN = '#00e68a', TEXT = '#e8eaf0', MUTED = '#505a6e';

const FACE_IN = 45.3, FACE_OUT = 52.86;
const FACE_CUTS = [FACE_IN, FACE_OUT];
const PUNCH: [number, number][] = [[47.6, 52.4]];
const CARDS = [
...
// Same 10 library transitions; from/to resolve to the VERTICAL deck/receipt PNGs (public-dir switch).
...
const CoverEl: React.FC<{ c: typeof ZCOVERS[number] }> = ({ c }) => {
  ...
  if (c.kind === 'chart') {
    if (c.ref === 'demand-vs-float') return <DemandVsFloatV />;
    if (c.ref === 'traction-scoreboard') return <TractionScoreboardV />;
    if (c.ref === 'buyback-flywheel') return <BuybackFlywheelV />;
...
const Captions: React.FC = () => {
  const t = useCurrentFrame() / ZV_FPS;
  ...
  const since = (t - cur.t) * ZV_FPS;
  const pop = interpolate(since, [0, 5, 9], [0.72, 1.12, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: ease });
  return (
    <AbsoluteFill style={{ justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 360, zIndex: 50 }}>
      <div style={{ fontFamily: "Montserrat,'Arial Black','Segoe UI',sans-serif", fontWeight: 900, fontSize: 84, color: '#fff', textTransform: 'lowercase', WebkitTextStroke: '11px #000', paintOrder: 'stroke fill', transform: `scale(${pop})`, textAlign: 'center' }}>{cur.h}</div>
    </AbsoluteFill>
  );
};
...
const TitleCard: React.FC<{ ey: string; title: string }> = ({ ey, title }) => {
  ...
  return (
    <AbsoluteFill style={{ background: '#0a0c10', justifyContent: 'center', alignItems: 'flex-start', padding: '0 72px', opacity: op }}>
      <div style={{ transform: `translateY(${y}px)` }}>
        ...
        <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 900, fontSize: 92, color: TEXT, lineHeight: 1.05, letterSpacing: '-.02em', maxWidth: 936 }}>{title}</div>
...
export const ZebecVertical: React.FC = () => {
  const f = useCurrentFrame();
  const t = f / ZV_FPS;
  let scale = 1;
  ...
  return (
    <AbsoluteFill style={{ background: '#000' }}>
      ...
      {TRANS.map((x, i) => {
        ...
        const win = framesForRow(row, ZV_FPS);
