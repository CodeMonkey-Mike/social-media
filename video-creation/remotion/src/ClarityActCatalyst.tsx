...
import {
  CAC_FPS, CAC_SEAM, CAC_CAP_Y, CLIP, THUMB, THUMB_TITLE, THUMB_CHIP,
  CAPTIONS_CAC, BROLL_CAC, SFX_CAC,
} from './constants-cac';
...
// visible below. The senate FULL hard-cuts into the desk ZONE (adjacent tOut/tIn <= EPS); every
// other beat has a base gap >> 1.5s so it fades to/from base (no base flash).
const BrollLayer: React.FC<{ t: number }> = ({ t }) => {
  const idx = BROLL_CAC.findIndex(e => t >= e.tIn && t < e.tOut);
  if (idx < 0) return null;
  const ev = BROLL_CAC[idx];
  const F = 0.12, EPS = 0.18;
  const prevAdj = BROLL_CAC.some((o, i) => i !== idx && Math.abs(o.tOut - ev.tIn) <= EPS);
  const nextAdj = BROLL_CAC.some((o, i) => i !== idx && Math.abs(o.tIn - ev.tOut) <= EPS);
  ...
  return (
    <AbsoluteFill style={{ opacity: op, zIndex: 60 }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 1080, height: CAC_SEAM, overflow: 'hidden' }}>
        <Img src={ev.src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${kb})` }} />
...
const Caption: React.FC = () => {
  ...
  const t = frame / CAC_FPS;
  let idx = 0, html = '';
  for (let i = CAPTIONS_CAC.length - 1; i >= 0; i--) {
    if (t >= CAPTIONS_CAC[i].t) { idx = i; html = CAPTIONS_CAC[i].h; break; }
  }
  if (!html) return null;
  const startFrame = Math.round((CAPTIONS_CAC[idx]?.t ?? 0) * CAC_FPS);
  const scale = spring({ frame: frame - startFrame, fps: CAC_FPS, config: { damping: 11, stiffness: 360 }, from: 0.72, to: 1.0 });
  return (
    <div style={{
      position: 'absolute', top: CAC_CAP_Y, left: 46, right: 46,
...
const ThumbCover: React.FC<{ op: number }> = ({ op }) => (
  <AbsoluteFill style={{ zIndex: 400, opacity: op }}>
    ...
    {/* Headline at top, chip near the bottom — so the Capitol/rocket subject stays visible between them. */}
...
// ─── Main composition ─────────────────────────────────────────────────────────────
export const ClarityActCatalyst: React.FC = () => {
  const frame = useCurrentFrame();
  const t = frame / CAC_FPS;
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT, overflow: 'hidden' }}>
      ...
      {/* Layer 1 — b-roll (full-screen at hook / Senate-pass transition / "send us flying" climax; zone
          on the desk-signing / institutions-holding-out / gates-open cutaways) */}
      <BrollLayer t={t} />

      {/* Layer 2 — caption legibility band straddling the seam */}
      <div style={{
        position: 'absolute', top: CAC_SEAM - 96, left: 0, right: 0, height: 216, zIndex: 120,
      ...
      {/* Layer 5 — SFX (whoosh / riser / impacts / ting / boom), each under the VO */}
      {SFX_CAC.map((s, i) => (
        <Sequence key={i} from={Math.round(s.t * CAC_FPS)} durationInFrames={Math.max(1, Math.round(s.dur * CAC_FPS))}>
