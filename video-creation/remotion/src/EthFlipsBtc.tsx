...
import {
  FPS_ETHF as FPS, CLIP_ETHF, LOGO_KAS,
  BROLL_ETHF, OVERLAYS_ETHF, FULLFACE_ETHF, GRAPHICS_ETHF, CAPTIONS_ETHF, SOUNDS_ETHF,
} from './constants-ethf';

const CONTENT_BOTTOM = 1010, CAPTION_Y = 968;
const TEAL = '#00e5ff', YELLOW = '#ffe600', GREEN = '#39ff14', RED = '#ff5252', ORANGE = '#f7931a', GREY = '#9aa3ad';
...
function getCaption(t: number) { for (let i = CAPTIONS_ETHF.length - 1; i >= 0; i--) if (t >= CAPTIONS_ETHF[i].t) return { idx: i, html: CAPTIONS_ETHF[i].h }; return { idx: 0, html: '' }; }
const Caption: React.FC<{ frame: number; t: number }> = ({ frame, t }) => {
  const { idx, html } = getCaption(t); if (!html) return null;
  const scale = spring({ frame: frame - Math.round((CAPTIONS_ETHF[idx]?.t ?? 0) * FPS), fps: FPS, config: { damping: 11, stiffness: 360 }, from: 0.7, to: 1.0 });
...
const Broll: React.FC<{ t: number }> = ({ t }) => {
  const ev = BROLL_ETHF.find(e => t >= e.tIn && t < e.tOut); if (!ev) return null;
...
const FullFace: React.FC<{ t: number }> = ({ t }) => {
  const ev = FULLFACE_ETHF.find(e => t >= e.tIn && t < e.tOut); if (!ev) return null;
  const op = fadeInOut(t, ev.tIn, ev.tOut, 0.15);
  return <AbsoluteFill style={{ opacity: op, overflow: 'hidden', background: '#000' }}><OffthreadVideo src={CLIP_ETHF} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scale(2.1)', transformOrigin: '50% 88%' }} /></AbsoluteFill>;
};

const Overlays: React.FC<{ t: number; frame: number }> = ({ t, frame }) => (
  <>{OVERLAYS_ETHF.map((e, i) => {
...
const Card: React.FC<{ op: number; sc: number; children: React.ReactNode; color?: string; top?: number }> = ({ op, sc, children, color = TEAL, top = 300 }) => (
  <div style={{ position: 'absolute', top, left: '50%', transform: `translate(-50%,-50%) scale(${sc})`, opacity: op, background: 'rgba(0,0,0,0.82)', border: `5px solid ${color}`, borderRadius: 28, padding: '34px 52px', textAlign: 'center', boxShadow: `0 0 56px ${color}66`, backdropFilter: 'blur(6px)', width: 880 }}>{children}</div>
);
...
const Thumb: React.FC<{ op: number }> = ({ op }) => (
  <AbsoluteFill style={{ opacity: op }}>
    ...
    <div style={{ position: 'absolute', top: 250, left: 50, right: 50, textAlign: 'center' }}>
      <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 88, lineHeight: 1.04, color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.01em', WebkitTextStroke: '4px #000', paintOrder: 'stroke fill' as any, whiteSpace: 'nowrap' }}>
        ETH FLIPS <span style={{ color: ORANGE }}>BTC</span><br />THEN <span style={{ color: TEAL }}>KASPA</span> FLIPS ETH
      </div>
      <div style={{ marginTop: 30, display: 'inline-block', background: TEAL, color: '#001016', fontFamily: FONT, fontWeight: 900, fontSize: 42, letterSpacing: '0.03em', padding: '16px 32px', borderRadius: 16, boxShadow: `0 0 40px ${TEAL}` }}>
        THE 10-YEAR ENDGAME
      </div>
    </div>
  </AbsoluteFill>
...
export const EthFlipsBtc: React.FC = () => {
  const frame = useCurrentFrame(); const t = frame / FPS;
  const g = (id: string) => GRAPHICS_ETHF.find(e => e.id === id);
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT }}>
      <AbsoluteFill style={{ overflow: 'hidden' }}><OffthreadVideo src={CLIP_ETHF} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></AbsoluteFill>
      <Broll t={t} />
      <FullFace t={t} />
      ...
      <AbsoluteFill style={{ zIndex: 100 }}>
        {gOp('ranking') > 0 && (
          <Card op={gOp('ranking')} sc={gSc('ranking')} color={GREEN} top={300}>
            <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 58, color: '#fff' }}>EVENTUALLY</div>
            <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 96, color: GREEN, textShadow: `0 0 28px ${GREEN}` }}>KASPA = #1</div>
          </Card>
        )}
        {gOp('t100') > 0 && (
          <Card op={gOp('t100')} sc={gSc('t100')} color={TEAL} top={300}>
            <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 120, color: TEAL, lineHeight: 1, textShadow: `0 0 36px ${TEAL}` }}>$100T</div>
            <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 32, color: '#fff', letterSpacing: '0.12em', marginTop: 8 }}>CRYPTO CATCHES STOCKS</div>
          </Card>
        )}
        {gOp('math') > 0 && (
          <Card op={gOp('math')} sc={gSc('math')} color={GREEN} top={300}>
            <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 88, color: '#fff' }}><span style={{ color: GREY }}>$3</span> &nbsp;→&nbsp; <span style={{ color: GREEN, textShadow: `0 0 28px ${GREEN}` }}>$30</span></div>
            <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 30, color: '#fff', letterSpacing: '0.1em', marginTop: 10 }}>$100B vs $1T MARKET CAP</div>
          </Card>
        )}
      </AbsoluteFill>
      ...
      {SOUNDS_ETHF.map((e, i) => (<Sequence key={i} from={Math.round(e.t * FPS)} durationInFrames={FPS * 2}><Audio src={e.src} volume={0.5} /></Sequence>))}
    </AbsoluteFill>
  );
};
