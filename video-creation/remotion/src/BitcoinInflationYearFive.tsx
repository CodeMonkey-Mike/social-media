...
import { FONT, TEAL, ORANGE, colourize, fadeInOut } from './_kit';
import {
  BIYF_FPS, BIYF_SEAM, BIYF_CAP_Y, CLIP_BIYF, THUMB_BIYF,
  CAPTIONS_BIYF, BROLL_BIYF, BADGES_BIYF, SFX_BIYF,
} from './constants-biyf';
...
// 'full' = whole frame (hook / thesis / doubled turn / illusion climax / close);
// 'zone' = the top screen-share zone only (0..BIYF_SEAM) so Mike's face stays visible below and the
// off-message static BTC/USD chart never shows. Adjacent beats hard-cut (no base flash); isolated
// beats fade. Palette is the molten-gold INFLATION world per BROLL-PLAN; the ONLY teal is the 5px seam.
const BrollLayer: React.FC<{ t: number }> = ({ t }) => {
  const idx = BROLL_BIYF.findIndex(e => t >= e.tIn && t < e.tOut);
  if (idx < 0) return null;
  const ev = BROLL_BIYF[idx];
  const F = 0.12, EPS = 0.18;
  const prevAdj = BROLL_BIYF.some((o, i) => i !== idx && Math.abs(o.tOut - ev.tIn) <= EPS);
  const nextAdj = BROLL_BIYF.some((o, i) => i !== idx && Math.abs(o.tIn - ev.tOut) <= EPS);
  ...
  return (
    <AbsoluteFill style={{ opacity: op, zIndex: 60 }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 1080, height: BIYF_SEAM, overflow: 'hidden' }}>
        <Img src={ev.src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${kb})` }} />
...
// amber (#ff9f1c) = dollar-price accent; white = percentages / years. Never over a full-screen peak,
// never over the caption band (y866). Time-separated. Big-text size adapts so long strings don't overflow.
const Badges: React.FC<{ t: number; fps: number }> = ({ t, fps }) => (
  <AbsoluteFill style={{ zIndex: 130 }}>
    {BADGES_BIYF.map((b, i) => {
      ...
      const color = b.color === 'amber' ? ORANGE : '#fff';
      const bigSize = b.big.length > 12 ? 58 : 84;
      return (
        <div key={i} style={{
          ...
          <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: bigSize, color, lineHeight: 0.92, textShadow: `0 0 30px ${color}`, whiteSpace: 'nowrap' }}>{b.big}</div>
...
const Caption: React.FC = () => {
  ...
  const t = frame / BIYF_FPS;
  let idx = 0, html = '';
  for (let i = CAPTIONS_BIYF.length - 1; i >= 0; i--) {
    if (t >= CAPTIONS_BIYF[i].t) { idx = i; html = CAPTIONS_BIYF[i].h; break; }
  }
  if (!html) return null;
  const startFrame = Math.round((CAPTIONS_BIYF[idx]?.t ?? 0) * BIYF_FPS);
  const scale = spring({ frame: frame - startFrame, fps: BIYF_FPS, config: { damping: 11, stiffness: 360 }, from: 0.72, to: 1.0 });
  return (
    <div style={{
      position: 'absolute', top: BIYF_CAP_Y, left: 46, right: 46,
...
// ─── Main composition ─────────────────────────────────────────────────────────────
export const BitcoinInflationYearFive: React.FC = () => {
  const frame = useCurrentFrame();
  const t = frame / BIYF_FPS;
  // Frame-0 designed-thumbnail cover (SKILL Phase 7 rule): opaque at frame 0, then a quick fade so it
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT, overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ overflow: 'hidden' }}>
        <OffthreadVideo src={CLIP_BIYF} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </AbsoluteFill>

      {/* Layer 1 — b-roll (full-screen at hook/thesis/turn/climax/close, zone cover over the static chart) */}
      <BrollLayer t={t} />

      {/* Layer 2 — caption legibility band straddling the seam */}
      <div style={{
        position: 'absolute', top: BIYF_SEAM - 96, left: 0, right: 0, height: 216, zIndex: 120,
      ...
      {!thumbUp && <Badges t={t} fps={BIYF_FPS} />}
      ...
      {thumbUp && (
        <AbsoluteFill style={{ zIndex: 400, opacity: thumbOp }}>
          <Img src={THUMB_BIYF} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </AbsoluteFill>
      )}

      {/* Layer 6 — SFX (whoosh / risers / impacts / cash / ding / ting / waitwhat), each under the VO */}
      {SFX_BIYF.map((s, i) => (
        <Sequence key={i} from={Math.round(s.t * BIYF_FPS)} durationInFrames={Math.max(1, Math.round(s.dur * BIYF_FPS))}>
