...
import {
  FPS_PYTHIA as FPS,
  CLIP_PYTHIA, LOGO_KAS,
  BROLL_PYTHIA, GRAPHICS_PYTHIA, CAPTIONS_PYTHIA, SOUNDS_PYTHIA,
} from './constants-pythia';
...
function getCaption(t: number) {
  for (let i = CAPTIONS_PYTHIA.length - 1; i >= 0; i--) {
    if (t >= CAPTIONS_PYTHIA[i].t) return { idx: i, html: CAPTIONS_PYTHIA[i].h };
  }
  return { idx: 0, html: '' };
}
...
const Caption: React.FC<{ frame: number; t: number }> = ({ frame, t }) => {
  ...
  if (!html) return null;
  const startFrame = Math.round((CAPTIONS_PYTHIA[idx]?.t ?? 0) * FPS);
...
const Broll: React.FC<{ t: number }> = ({ t }) => {
  const ev = BROLL_PYTHIA.find(e => t >= e.tIn && t < e.tOut);
  if (!ev) return null;
  ...
  <AbsoluteFill style={{ opacity: op }}>
    ...
    <div style={{ position: 'absolute', top: 220, left: 60, right: 60, textAlign: 'center' }}>
      <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 120, lineHeight: 0.98, color: '#fff', textTransform: 'uppercase', WebkitTextStroke: '4px #000', paintOrder: 'stroke fill' as any }}>
        MY COMMUNITY<br />JUST <span style={{ color: GREEN }}>28X'D</span><br />ON A <span style={{ color: TEAL }}>RAT</span>
      </div>
      <div style={{ marginTop: 30, display: 'inline-block', background: TEAL, color: '#001016', fontFamily: FONT, fontWeight: 900, fontSize: 44, letterSpacing: '0.04em', padding: '16px 36px', borderRadius: 16, boxShadow: `0 0 40px ${TEAL}` }}>
        PYTHIA — TAO + NEUROSCIENCE
      </div>
    </div>
  </AbsoluteFill>
);

export const Pythia28x: React.FC = () => {
  const frame = useCurrentFrame();
  const t = frame / FPS;
  const g = (id: string) => GRAPHICS_PYTHIA.find(e => e.id === id);
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT }}>
      <AbsoluteFill style={{ overflow: 'hidden' }}>
        <OffthreadVideo src={CLIP_PYTHIA} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </AbsoluteFill>
      <Broll t={t} />
      ...
      <AbsoluteFill style={{ zIndex: 100 }}>
        <Badge op={gOp('badge-28x')}         sc={gSc('badge-28x')}         color={GREEN} line1="28X"          sub="ON PYTHIA" />
        <Badge op={gOp('badge-defi-neuro')}  sc={gSc('badge-defi-neuro')}  color={TEAL}  line1="DEFI +"      line2="NEUROSCIENCE" />
        <Badge op={gOp('badge-comeback')}    sc={gSc('badge-comeback')}    color={GREEN} line1="COMEBACK"    sub="WATCH THIS THING" />
        <Badge op={gOp('badge-community')}   sc={gSc('badge-community')}   color={TEAL}  line1="MY COMMUNITY" line2="WON" />
      </AbsoluteFill>
      ...
      {t < 6.5 && <AbsoluteFill style={{ zIndex: 300 }}><Thumb op={interpolate(t, [0, 6.2, 6.5], [1, 1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })} /></AbsoluteFill>}

      {SOUNDS_PYTHIA.map((e, i) => (
