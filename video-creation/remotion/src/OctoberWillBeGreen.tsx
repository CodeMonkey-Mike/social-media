...
import {
  OWBG_FPS, OWBG_SEAM, OWBG_CAP_Y, CLIP_OWBG, THUMB_OWBG,
  CAPTIONS_OWBG, BROLL_OWBG, BADGES_OWBG, SFX_OWBG,
} from './constants-owbg';
...
// 'full' = whole frame (hook / green-regardless payoff / crossroads climax / newlevel close);
// 'zone' = the top screen-share zone only (0..OWBG_SEAM) so Mike's face stays visible below and the
// bearish BTC chart + ticker never show. Adjacent beats hard-cut (no base flash); isolated beats fade.
// Palette is the acid neon-green PUMP world per BROLL-PLAN; the ONLY teal is the 5px brand seam line.
const BrollLayer: React.FC<{ t: number }> = ({ t }) => {
  const idx = BROLL_OWBG.findIndex(e => t >= e.tIn && t < e.tOut);
  if (idx < 0) return null;
  const ev = BROLL_OWBG[idx];
  const F = 0.12, EPS = 0.18;
  const prevAdj = BROLL_OWBG.some((o, i) => i !== idx && Math.abs(o.tOut - ev.tIn) <= EPS);
  const nextAdj = BROLL_OWBG.some((o, i) => i !== idx && Math.abs(o.tIn - ev.tOut) <= EPS);
  ...
  return (
    <AbsoluteFill style={{ opacity: op, zIndex: 60 }}>
      <div style={{ position: 'absolute', top: 0, left: 0, width: 1080, height: OWBG_SEAM, overflow: 'hidden' }}>
        <Img src={ev.src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${kb})` }} />
...
// green = pump world; teal = brand thread on the open-loop NEW BOTTOM. Never over a full-screen peak,
// never over the caption band (y866). Time-separated from one another.
const Badges: React.FC<{ t: number; fps: number }> = ({ t, fps }) => (
  <AbsoluteFill style={{ zIndex: 130 }}>
    {BADGES_OWBG.map((b, i) => {
      ...
      const color = b.color === 'teal' ? TEAL : GREEN;
      return (
        <div key={i} style={{
          ...
          <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 84, color, lineHeight: 0.92, textShadow: `0 0 30px ${color}`, whiteSpace: 'nowrap' }}>{b.big}</div>
...
const Caption: React.FC = () => {
  ...
  const t = frame / OWBG_FPS;
  let idx = 0, html = '';
  for (let i = CAPTIONS_OWBG.length - 1; i >= 0; i--) {
    if (t >= CAPTIONS_OWBG[i].t) { idx = i; html = CAPTIONS_OWBG[i].h; break; }
  }
  if (!html) return null;
  const startFrame = Math.round((CAPTIONS_OWBG[idx]?.t ?? 0) * OWBG_FPS);
  const scale = spring({ frame: frame - startFrame, fps: OWBG_FPS, config: { damping: 11, stiffness: 360 }, from: 0.72, to: 1.0 });
  return (
    <div style={{
      position: 'absolute', top: OWBG_CAP_Y, left: 46, right: 46,
...
// ─── Main composition ─────────────────────────────────────────────────────────────
export const OctoberWillBeGreen: React.FC = () => {
  const frame = useCurrentFrame();
  const t = frame / OWBG_FPS;
  ...
  return (
    <AbsoluteFill style={{ background: '#000', fontFamily: FONT, overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ overflow: 'hidden' }}>
        <OffthreadVideo src={CLIP_OWBG} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </AbsoluteFill>

      {/* Layer 1 — b-roll (full-screen at hook/payoff/climax/close, zone cover over the bearish chart) */}
      <BrollLayer t={t} />

      {/* Layer 2 — caption legibility band straddling the seam */}
      <div style={{
        position: 'absolute', top: OWBG_SEAM - 96, left: 0, right: 0, height: 216, zIndex: 120,
      ...
      {!thumbUp && <Badges t={t} fps={OWBG_FPS} />}
      ...
      {thumbUp && (
        <AbsoluteFill style={{ zIndex: 400, opacity: thumbOp }}>
          <Img src={THUMB_OWBG} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </AbsoluteFill>
      )}

      {/* Layer 6 — SFX (whoosh / risers / impacts / cash / ding / ting / waitwhat), each under the VO */}
      {SFX_OWBG.map((s, i) => (
        <Sequence key={i} from={Math.round(s.t * OWBG_FPS)} durationInFrames={Math.max(1, Math.round(s.dur * OWBG_FPS))}>
