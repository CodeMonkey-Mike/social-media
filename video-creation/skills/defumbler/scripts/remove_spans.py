...
A/V locked. Single-pass filter_complex (trim+atrim+concat), so audio and video cannot drift.
Cuts are the COMPLEMENT of the kept spans over [0, duration]. 8ms declick fades on every join.

DEFUMBLER RULE: every --cut span you pass here MUST start and end INSIDE a silence gap (use the
sil_before/sil_after from chunk_map's .json). Never pass a span whose edge lands on a spoken word —
that is what clips words. See defumbler.md.
...
def main():
    ...
    ap.add_argument("--cuts-file", help="text file with one 'start-end' span per line (for large cut lists)")
    ...
    spans = list(args.cut)
    if args.cuts_file:
        with open(args.cuts_file, encoding="utf-8") as f:
            spans += [ln.strip() for ln in f if ln.strip() and "-" in ln]
    cuts = []
    for c in spans:
        a, b = c.split("-")
    ...
    with open(args.out + ".spans.json", "w", encoding="utf-8") as f:
        json.dump({"src": args.src, "cuts": cuts, "keeps": keeps}, f, indent=1)
