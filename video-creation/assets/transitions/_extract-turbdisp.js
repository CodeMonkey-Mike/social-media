#!/usr/bin/env node
/** Per-clip real data for all 10 Turbulent Displace transitions (1x-7x).
 ...
 * Writes _turbdisp-clips.json. Reads decompressed project XML from %TEMP%/sw.xml
...
const NAMES = [];
for (const d of ['Horizontal','Vertical']) for (const n of [1,2,3,4,5]) NAMES.push(`Turbulent Displace ${d} - ${n}x`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_turbdisp-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _turbdisp-clips.json\n');
