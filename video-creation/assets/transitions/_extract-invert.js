#!/usr/bin/env node
/** Per-clip real data for all 9 Glitch Invert transitions (Max/Min/Short x 1-3).
 ...
 * Writes _invert-clips.json. Reads decompressed project XML from %TEMP%/sw.xml
...
const NAMES = [];
for (const v of ['Max', 'Min', 'Short']) for (const n of [1, 2, 3]) NAMES.push(`Glitch Invert ${v} - ${n}`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_invert-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _invert-clips.json\n');
