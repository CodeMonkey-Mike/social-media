#!/usr/bin/env node
/** Per-clip real data for all 8 Glitch Monitor transitions (Glitch Monitor - 1..8).
 * Adapted from _extract-monitor.js (same per-clip method; see that file for notes). */
const fs = require('fs');
...
const NAMES = [];
for (const n of [1, 2, 3, 4, 5, 6, 7, 8]) NAMES.push(`Glitch Monitor - ${n}`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_glitchmonitor-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _glitchmonitor-clips.json\n');
