#!/usr/bin/env node
/** Per-clip real data for all 7 Glitch Offset transitions (1x-7x).
 ...
 * Writes _offset-clips.json. Reads decompressed project XML from %TEMP%/sw.xml
...
const NAMES = [];
for (const n of [1, 2, 3, 4, 5, 6, 7]) NAMES.push(`Glitch Offset - ${n}x`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_offset-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _offset-clips.json\n');
