#!/usr/bin/env node
/** Per-clip real data for all 9 TV Satellite transitions (1x-7x).
 ...
 * Writes _tvsat-clips.json. Reads decompressed project XML from %TEMP%/sw.xml
...
const NAMES = [];
for (const v of ['Max','Min','Short']) for (const n of [1,2,3]) NAMES.push(`Glitch TV Satellite ${v} - ${n}`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_tvsat-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _tvsat-clips.json\n');
