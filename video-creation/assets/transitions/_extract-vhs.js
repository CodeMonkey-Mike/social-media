#!/usr/bin/env node
/** Per-clip real data for all 9 VHS transitions (1x-7x).
 ...
 * Writes _vhs-clips.json. Reads decompressed project XML from %TEMP%/sw.xml
...
const NAMES = [];
for (const v of ['Max','Min','Short']) for (const n of [1,2,3]) NAMES.push(`Glitch VHS ${v} - ${n}`);
const out = [];
...
fs.writeFileSync(path.join(__dirname, '_vhs-clips.json'), JSON.stringify(out, null, 2));
process.stdout.write('DONE -> _vhs-clips.json\n');
