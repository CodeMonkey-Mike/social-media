 ...
 * param (static value + ALL keyframes). Writes _expandpan-clips.json.
...
const NAMES = [];
for (const f of ["Expand Pan","Expand Pan Short"]) for (const d of ["Up","Down","Left","Right"]) NAMES.push(f+" - "+d);
...
const outfile = filter ? '_expandpan-clips.json' : '_expandpan-clips.json';
