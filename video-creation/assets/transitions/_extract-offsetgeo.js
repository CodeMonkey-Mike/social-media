#!/usr/bin/env node
/** Per-clip real data for the whole OFFSET (motion push/slide) category:
 *   19 sub-families x 8 directions = 152 sequences.
 * Same closure-walk as _extract-offset.js (the GLITCH Offset extractor) — sequences
 * carry TWO TrackGroups and the audio one can come first (pick by tag), clips carry
 * media in-points, effects apply BOTTOM-UP. Per clip: time window, SubClip->MasterClip
 * (name / adjustment flag / media path), media in-point, every component with every
 * param (static value + ALL keyframes). Writes _offsetgeo-clips.json.
 * Reads decompressed project XML from %TEMP%/sw.xml.
 *
 * Optional CLI arg = a substring filter to extract just a sample (e.g. "Simple - Right").
 */
const fs = require('fs');
  ...
  while ((pm = pr.exec(compXml))) {
    ...
    if (kfRaw) {
      entry.keyframes = kfRaw.split(';').filter(Boolean).map((k) => {
        ...
        // (decoded 2026-07-11 — the vel/inf fields are the AE temporal-bezier handles;
        // discarding them collapsed every eased curve to linear. Keep ALL fields.)
...
const SUBFAMS = [
  'Offset Simple', 'Offset Simple Short', 'Offset Long Simple',
  'Offset Ease', 'Offset Ease Short', 'Offset Ease Out', 'Offset Ease Out Short',
  'Offset Long Ease', 'Offset Long Ease Out',
  'Offset Bounce', 'Offset Bounce Short',
  'Offset Hit', 'Offset Hit Short', 'Offset Long Hit',
  'Offset Swinging', 'Offset Swinging Short', 'Offset Long Swinging',
  'Offset Warp', 'Offset Warp Short',
];
const DIRS = ['Up', 'Down', 'Left', 'Right', 'Left Up', 'Left Down', 'Right Up', 'Right Down'];

const filter = process.argv[2];
const NAMES = [];
for (const sf of SUBFAMS) for (const d of DIRS) {
  const nm = `${sf} - ${d}`;
  if (!filter || nm.includes(filter)) NAMES.push(nm);
}
...
const outfile = filter ? '_offsetgeo-sample.json' : '_offsetgeo-clips.json';
